...
const obtainRateOfClimbGraphicByAircraft = async (idAircraft: string, segment: number, state: string) => {
    const response = await RateOfClimbGrpahicModel.findOne({ aircraft: { $in: [idAircraft] }, segment: segment, state: state });
    return response;
}
